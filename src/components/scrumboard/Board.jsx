...
const Board = props => {
  ...
  const deleteSubtaskHandler = (subtaskId, taskId) => {
    ...
    const alltasks = tasks;
    let index = alltasks.findIndex(task => task.id === taskId);
    const allSubtask = alltasks[index].subtasks.filter(a => a.id !== subtaskId);
  ...
  const changeStatusOfSubtaskHandler = (subtaskId, taskId) => {
    ...
    alltasks[taskIndex].subtasks[subtaskIndex].completed = !alltasks[taskIndex]
      ...
      <div className="flex-y">
        <div className="mlr-8 flex-x board-toolbar pa-10">
          <div className="fs-16 bold-text board-title flex-1">
            <i className="fas fa-briefcase mr-10"></i>
            {activeBoard && activeBoard.title}
          </div>
          <div
            className="fs-16 bold-text cursor-pointer board-action"
            onClick={() => props.history.push("/scrumboard")}
          >
        ...
        <div className="flex-1">
          <DragDropContext onDragEnd={onDragEnd}>
            <Droppable droppableId="card" type="card" direction="horizontal">
              {provided => (
                <div ref={provided.innerRef} className="scrum-board">
                  {boardCards &&
                  ...
                  <BoardCardAdder addNewCardHandler={addNewCardHandler} />
                </div>
              )}
            </Droppable>
          </DragDropContext>
